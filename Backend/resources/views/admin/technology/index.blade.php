    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-stone-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    <table class="w-full text-sm text-left text-stone-500 dark:text-stone-400">
                        ...
                        @foreach($technology as $techno)
                            <tr class=" bg-stone-100 dark:border-stone-700  hover:transition {{$techno?->deleted_at ? " dark:bg-teal-900 hover:bg-teal-950" : "dark:bg-stone-800 hover:bg-stone-900"}}">
                                <td class="px-6 py-4 text-sm leading-5 whitespace-nowrap">
                                    {{$techno->id}}
                                </td>
                                <td class="px-6 py-4 text-sm leading-5 whitespace-nowrap">
                                    <img src="{{Storage::url($techno->image)}}" alt="" width="100px">
                                </td>
                                <td class="px-6 py-4 text-sm leading-5 whitespace-nowrap">
                                    {{$techno->name}}
                                </td>
                                ...
                                <td class="px-6 py-4 text-sm leading-5 whitespace-nowrap flex flex-col gap-2 justify-end">
                                    ...
                                    <form action="{{ route('admin.technology.edit', $techno) }}" class="inline">
                                        @csrf
                                    ...
                                    <form 
                                        action="{{ route("admin.technology.delete", $techno) }}" 
                                        method="POST" 
                                        onsubmit="return confirm('Are you sure you want to delete {{$techno->name}} ? ');" 
                                        class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <input 
                ...
                <div class="px-6 mt-8">{{$technology->links()}}</div>
            </div>
        </div>
    </div>
</x-app-layout>