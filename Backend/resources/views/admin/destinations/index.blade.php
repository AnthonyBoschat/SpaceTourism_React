    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-stone-800 overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <table class="w-full text-sm text-left text-stone-500 dark:text-stone-400">
                        ...
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-center">
                                <div class="flex items-center">
                                    <span
                                        class="text-xs font-medium uppercase leading-4 tracking-wider text-stone-500 dark:text-white ">{{__("Description")}}</span>
                                </div>
                            </th>
                            <th scope="col" class="px-6 py-3 text-center">
                                <div class="flex items-center">
                                    <span
                                        class="text-xs font-medium uppercase leading-4 tracking-wider text-stone-500 dark:text-white ">{{__("Distance")}}</span>
                                </div>
                            </th>
                            <th scope="col" class="px-6 py-3 text-center">
                                <div class="flex items-center">
                                    <span
                                        class="text-xs font-medium uppercase leading-4 tracking-wider text-stone-500 dark:text-white ">{{__("TravelTime")}}</span>
                                </div>
                            </th>
                        ...
                        @foreach($destinations as $destination)
                            <tr class=" bg-stone-100 dark:border-stone-700  hover:transition {{$destination?->deleted_at ? " dark:bg-teal-900 hover:bg-teal-950" : "dark:bg-stone-800 hover:bg-stone-900"}}">
                                <td class="px-6 py-4 text-sm leading-5 whitespace-nowrap">
                                    {{$destination->id}}
                                </td>
                                <td class="px-6 py-4 text-sm leading-5 whitespace-nowrap">
                                    <img src="{{Storage::url($destination->image)}}" alt="" width="100px">
                                </td>
                                <td class="px-6 py-4 text-sm leading-5 whitespace-nowrap">
                                    {{$destination->name}}
                                </td>
                                ...
                                <td class="px-6 py-4 text-sm leading-5 whitespace-nowrap flex flex-col gap-2 justify-end">
                                    ...
                                    <form action="{{ route('admin.destinations.edit', $destination) }}" class="inline">
                                        @csrf
                                    ...
                                    <form 
                                        action="{{ route("admin.destinations.delete", $destination) }}" 
                                        method="POST" 
                                        onsubmit="return confirm('Are you sure you want to delete {{$destination->name}} ? ');" 
                                        class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <input 
                ...
                <div class="px-6 mt-8">{{$destinations->links()}}</div>
            </div>
        </div>
    </div>
</x-app-layout>